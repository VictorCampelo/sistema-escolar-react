...
const WriteOffBillets = ({ docId }) => {
          ...
          if (snapshot.exists()) {
            for (const docKey in snapshot.val()) {
              if (Object.hasOwnProperty.call(snapshot.val(), docKey)) {
                ...
                localDoc.hasOwnProperty("historico")
                  ? setBilletHistory(localDoc.historico)
                  : setBilletHistory();
              }
            }
            ...
            setDialog(true);
            enqueueSnackbar("Boleto não encontrado no banco de dados.", {
              title: "Erro",
              variant: "error",
              key: "0",
              action:
  ...
  const handleWriteOff = async () => {
    ...
    const text = await confirm({
      ...
      title: "Confirmação",
      description:
        "Você deseja dar baixa neste boleto? (Após dar baixa, pode ser que o status do boleto não mude de imediato. Fique tranquilo, o sistema está processando a baixa em background, você não precisa esperar.)",
      promptText: true,
    ...
    if (dueDate < paymentDay) {
      ...
      await billetsDocsRef
        ...
        .push({
          ...
          userCreator: user.id,
          motivo: text === undefined ? "0" : text
        });
      enqueueSnackbar("Baixa solicitada. O sistema processará o pedido em background.", {
        title: "Sucesso",
        variant: "info",
        key: "0",
        action:
  ...
  const handleChallenge = async () => {
    const text = await confirm({
      ...
      title: "Confirmação",
      description:
        "Você deseja contestar este boleto? Se sim, digite abaixo o motivo e clique em 'Sim'. Sua decisão pode estar sujeita a aprovação.",
      promptText: true,
    ...
    await billetsDocsRef
      .child(docKeyPath)
      .child("historico")
      .push({ status: 3, motivo: text, userCreator: user.id });
    enqueueSnackbar("Contestação solicitada. O sistema processará o pedido em background.", {
      title: "Sucesso",
      variant: "info",
      key: "0",
      action:
  ...
  const handleUnChallenge = async () => {
    const text = await confirm({
      ...
      title: "Confirmação",
      description:
        "Você deseja retirar a contestação deste boleto? Se sim, digite abaixo o motivo e clique em 'Sim'. Sua decisão pode estar sujeita a aprovação.",
      promptText: true,
    ...
    await billetsDocsRef
      .child(docKeyPath)
      .child("historico")
      .push({ status: 0, motivo: text, userCreator: user.id });
    enqueueSnackbar(
      "Remoção de contestação solicitada. O sistema processará o pedido em background.",
      {
        title: "Sucesso",
        variant: "info",
        key: "0",
  ...
  const handleCancel = async () => {
    ...
    await billetsDocsRef
      .child(docKeyPath)
      .child("historico")
      .push({ status: 4, motivo: text, userCreator: user.id });
    enqueueSnackbar("Cancelamento solicitado. O sistema processará o pedido em background.", {
      title: "Sucesso",
      variant: "info",
      key: "0",
      action:
    ...
    <Fragment>
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} aria-labelledby="form-dialog-title">
        ...
        <form onSubmit={(e) => handleChangeDocId(e, localDocIdTemp)} id={"formSearchBillet"}>
          <DialogContent>
            <DialogContentText>
              Digite o número do Documento que se encontra em cada parcela para que o sistema
              ...
              <CardContent>
                ...
                <Box m={1}>
                  <Typography variant="subtitle1" component={"label"}>
                    Status:{" "}
                    <label style={{ color: billetColors[doc.status] }}>
                      {billetStatus[doc.status]}
                    </label>
                  </Typography>
