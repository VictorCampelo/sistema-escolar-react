          ...
          <Grid item>
            <FormControl className={S.fields}>
              <TextField
                ...
                InputLabelProps={{ shrink: shrink }}
                variant="filled"
                label="Senha do portal do Professor"
                type="password"
                id="senhaProfessor"
                name="senhaProfessor"
                aria-describedby="my-helper-text"
...
function CourseDataFields(props) {
  ...
  const handleRowClick = (data) => {
    ...
    setCourseChosen(coursesData.filter((course) => course.turmaProfessor === data.id)[0]);
    sessionStorage.setItem(
      activeStep,
      JSON.stringify({
        dadosTurma: coursesData.filter((course) => course.turmaProfessor === data.id)[0]
      })
    );
    setOpenDialog(true);
  };
...
const AddressAndParentsFields = ({ shrink, parentsRequired, editMode = false }) => {
  ...
  const handleCloseDialog = () => {
    if (parentsFields.length === 0 && parentsRequired) {
      enqueueSnackbar(
        "O Professor é menor de idade. Deve ser cadastrado pelo menos um responsável.",
        {
          variant: "warning",
          key: "0",
          action:
      ...
      <div className={S.root}>
        ...
        {additionalFields &&
          additionalFields.map((field) =>
            <FormControl key={field.id} className={S.fields} style={{ width: "100%" }}>
              <TextField
