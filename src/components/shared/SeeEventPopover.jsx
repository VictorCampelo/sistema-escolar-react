...
import { Close, Delete, Edit, PersonOutline } from "@material-ui/icons";
...
const SeeEventPopover = ({
  ...
  useEffect(() => {
    if (isFromClassCode) {
      classesRef
        .child(eventId)
        .child("frequencia")
        .child(event.startStr)
        .on("value", async (faultsSnap) => {
  ...
  const getClassFaults = async () => {
    const faultsSnap = await classesRef
      .child(eventId)
      .child("frequencia")
      .child(event.startStr)
      .once("value");
  ...
  const handleDeleteEvent = async () => {
    try {
      await confirm({
        ...
        title: "Confirmação",
        description:
          "Você deseja deletar este evento? Este evento e todas as suas ocorrências serão apagadas."
      });
      handleClose();
  ...
  const handleRemoveFault = async (fault) => {
    ...
    handleFault(
      {
        eventStr: event.startStr,
        classCode: eventId,
        studentId: fault.id,
        studentName: fault.name
      },
      true
    );
  };
        ...
        <DialogContent>
            ...
            <>
              ...
              <Typography variant="p" component={"p"}>
                <Typography variant="label">
                  {daysOfWeek[event.start.getDay()]}, {event.start.toLocaleDateString()}
                </Typography>
          ...
          {recurrence &&
            <Typography variant="p">
              ...
              {recurrence.daysOfWeek && recurrence.endRecur
              ...
              <List>
                {faults.map((fault, i) =>
                  <ListItem>
                    ...
                    <ListItemText primary={fault.name} />
                    <ListItemSecondaryAction>
                      <Tooltip title={"Remover falta"}>
                        <IconButton
                          edge="end"
                          aria-label="delete"
