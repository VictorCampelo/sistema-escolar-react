...
export default function StudentFiles({ studentId, disabledStudent, preEnrollment }) {
  ...
  const getData = async () => {
    setLoading(true);
    if (preEnrollment) {
      const snapshot = await preEnrollmentsRef
        .child(studentId)
        .child("studentFilesKey")
        .once("value");
      if (snapshot.exists()) {
        setStudentFiles();
      ...
      const snapshot = disabledStudent
        ? await disabledStudentsRef
            .child(studentId)
            .child("dadosAluno")
            .child("studentFilesKey")
            .once("value")
        : await studentsRef.child(studentId).child("studentFilesKey").once("value");
      ...
      if (snapshot.exists()) {
        ...
        disabledStudent
          ? disabledStudentsRef
              ...
              .child("studentFilesKey")
              .set(newKey)
          : studentsRef.child(studentId).child("studentFilesKey").set(newKey);
        getData();
      }
    }
  ...
  const ListItemElem = (props) => {
    ...
    return (
      <ListItem button={button} onClick={onClick}>
        <ListItemText primary={name} secondary={secondary ? formatBytes(size) : null} />
    ...
    <div className={S.root}>
      ...
      <Grid container spacing={1}>
        <Grid item xs={12} md={12}>
          ...
          <Typography color="textSecondary">Armazenamento utilizado: {totalFilesSize}</Typography>
        </Grid>
      </Grid>
    </div>
  );
