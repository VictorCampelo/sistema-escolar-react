...
export default function ClassReportOLD({ classCode, open, onClose }) {
  ...
  function setRows(rows) {
    ...
    rows.map((row) => {
      cells = "";
      row.map((cell) => {
        cells += `<td className="tg-0lax" style="${cell.center && "text-align: center;"
  ...
  function infoTopics(topics) {
    ...
    topics.map((topic) => {
      infoTopicsRow.innerHTML += `
            <td className="tg-0pky" style="${topic.center && "text-align: center;"}" colSpan="${topic.size
        }">${topic.text}</td>
        `;
    });
  }
  ...
  async function geraDiario(turma) {
    setLoader(true);
      ...
      if (Object.hasOwnProperty.call(alunos, matricula)) {
        ...
        for (const nomeNota in notas) {
          try {
            ...
            infoRow.push({
              text: nota === 0 || nota === undefined ? " " : nota,
              size: 1,
              center: true
            });
            somatorioNotas += nota;
        ...
        infoRow.push({
          text: somatorioNotas === 0 || isNaN(somatorioNotas) ? " " : somatorioNotas,
          size: 1,
          center: true
        });
