...
import { Add, Delete, Receipt, Refresh } from "@material-ui/icons";
...
const StudentContracts = ({ studentId, isOpen, onClose, isDisabled }) => {
  ...
  const getData = async () => {
    ...
    const snapshot = !isDisabled
      ? await studentsRef.child(studentId).child("contratos").once("value")
      : await disabledStudentsRef
        .child(studentId + "/dadosAluno")
        .child("contratos")
        .once("value");
  ...
  useEffect(() => {
    ...
    if (contractCode && configuredContract && originalPlan) {
      contractRef
        ...
        .then(() => {
          setLoading(true);
          enqueueSnackbar("Contrato criado com sucesso! Aguarde um pouco, e clique em atualizar.", {
            title: "Sucesso",
            variant: "success",
            key: "0",
            persist: true,
    ...
    if (!isDisabled) {
      try {
        ...
        await confirm({
          ...
          title: "Confirmação",
          description:
            "Você deseja deletar e cancelar este contrato. Ao deletar este contrato ele automaticamente será cancelado, e se houver boletos com status 'Pendente' os mesmos serão mudados para o status cancelado."
        });
        ...
        enqueueSnackbar(
          "Exclusão de contrato solicitada. O sistema está cancelando e deletando o contrato em background.",
          {
            title: "Sucesso",
            variant: "info",
            persist: true,
      ...
      enqueueSnackbar("Não é possível deletar enquanto o aluno está desativado", {
        title: "Erro",
        variant: "info",
        key: "0",
        action:
  ...
  const handleGenerateBillets = async (contractId) => {
    try {
      ...
      localContract[0].hasOwnProperty("docsBoletos") &&
        await confirm({
          variant: "danger",
          catchOnCancel: true,
          title: "Confirmação",
          description:
            "Já existem boletos gerados para este contrato, se quiser vê-los, cancele esta ação e clique na opção de Segunda Via. Se continuar, serão gerados novos débitos/boletos para este contrato. Deseja continuar?"
        });
  ...
  const handleOnlyShowBillets = (contractId) => {
    ...
    if (localContract[0].hasOwnProperty("docsBoletos")) {
      ...
      enqueueSnackbar("Não existem boletos para este contrato. Clique em \"Gerar Boletos\".", {
        title: "Info",
        variant: "info",
        key: "0",
        action:
        ...
        <Card className={S.smallCards} variant="outlined">
          <CardContent>
            <Grid justifyContent="space-between" direction="row" container spacing={1}>
              ...
              <Grid item>
                <IconButton
                  color="secondary"
                  style={{ float: "right" }}
                  disabled={Object.keys(contracts).length === 1}
            ...
            <Button
              ...
              variant="contained"
              color="primary"
              onClick={() => handleOnlyShowBillets(contract.codContrato)}>
  ...
  return (
    <Fragment>
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        ...
        <DialogContent>
          <DialogContentText>Selecione o curso que o aluno irá fazer:</DialogContentText>
          <FormControl fullWidth>
            ...
            <Select
              ...
              fullWidth>
              {schoolCourses &&
                schoolCourses.map((localCourse, i) =>
                  <MenuItem value={JSON.stringify({ dadosTurma: { courseId: localCourse.id } })}>
                    {localCourse.nomeCurso}
                  </MenuItem>
                )}
            </Select>
          </FormControl>
      ...
      {newContract &&
        <ContractConfigure
          activeStep={"newContract"}
          isOpen={newContract}
          setOpenDialog={setNewContract}
        />
      }
