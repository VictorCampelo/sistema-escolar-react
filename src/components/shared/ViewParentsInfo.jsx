...
import { Add, Delete, SupervisedUserCircle } from "@material-ui/icons";
  ...
  const getData = async () => {
    if (preEnrollment) {
      ...
      const snapshot = !isDisabled
        ? await studentsRef.child(studentId).child("responsaveis").once("value")
  ...
  const handleSaveData = async (e) => {
    ...
    try {
      if (edit && Object.keys(parents).length > 0) {
        ...
        preEnrollment
          ? await preEnrollmentsRef.child(studentId).child("responsaveis").set(parents)
          : await studentsRef.child(studentId).child("responsaveis").set(parents);
        enqueueSnackbar("Responsáveis atualizados com sucesso.", {
          title: "Sucesso",
          variant: "success",
          key: "0",
          action:
...
try {
        ...
        preEnrollment
          ? await preEnrollmentsRef.child(studentId).child("responsaveis").child(key).remove()
          : await studentsRef.child(studentId).child("responsaveis").child(key).remove();
        enqueueSnackbar("Responsável excluído com sucesso.", {
          title: "Sucesso",
          variant: "success",
          key: "0",
          action:
...
enqueueSnackbar("Não é possível apagar enquanto o aluno está desativado", {
        title: "Erro",
        variant: "info",
        key: "0",
        action:
          ...
          <CardContent>
              ...
              <Grid item>
                <div onMouseEnter={() => setMouseOver(i)} onMouseLeave={() => setMouseOver(false)}>
                  {mouseOver === i ?
                    <IconButton
                      color="primary"
                      disabled={Object.keys(parents).length === 1}
            ...
            <Box m={1}>
              <FormControl fullWidth size="small" variant="filled">
                <InputLabel htmlFor="relacao" style={{ marginTop: "16px" }} required shrink={true}>
                  Relação
                </InputLabel>
                <Select
                  disabled={!edit}
...
enqueueSnackbar("Não é possível editar enquanto o aluno está desativado", {
              title: "Erro",
              variant: "info",
              key: "0",
              action:
