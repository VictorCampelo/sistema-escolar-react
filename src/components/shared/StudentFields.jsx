...
function BasicDataFields({
  ...
  const [enrollType, setEnrollType] = useState(
    external ? { checked: true, value: "preMatricula" } : { checked: false, value: "matricula" }
  );
  ...
  const handleChangeEnrollType = () => {
    setEnrollType({
      checked: !enrollType.checked,
      value: !enrollType.checked ? "preMatricula" : "matricula"
    });
            ...
            <FormGroup>
              <FormControlLabel
                control={
                  <Switch
                    checked={enrollType.checked}
                    onChange={handleChangeEnrollType}
                    id="tipoMatricula"
                    name="tipoMatricula"
                    value="preMatricula"
                    color="primary"
          ...
          <Grid item>
            <FormControl className={S.fields}>
              <TextField
                ...
                name="telefoneAluno"
                aria-describedby="my-helper-text"
                onKeyPress={(e) => $(e.target).mask("99999999999")}
              />
            </FormControl>
          </Grid>
          <Grid item>
          ...
          <Grid item>
            <FormControl className={S.fields}>
              <TextField
                ...
                InputLabelProps={{ shrink: shrink }}
                variant="filled"
                label="Senha do portal do Aluno"
                type="password"
                id="senhaAluno"
                name="senhaAluno"
                aria-describedby="my-helper-text"
          ...
          <Grid item>
            <FormControl className={S.fields}>
              <TextField
                ...
                onBlur={() => {
                  if (validCpf) {
                    document.getElementById("cpfAluno").value = null;
                  }
                }}
                type="text"
                id="cpfAluno"
                name="cpfAluno"
                aria-describedby="my-helper-text"
                helperText={validCpf && "Insira um CPF válido."}
  ...
  const contractHandler = async (save = false) => {
    ...
    if (Number(internData.numeroParcelas) > Number(internPlan.numeroMaximoParcelasPlano)) {
      setOpenDialogError({
        ...
        message:
          "Este plano permite apenas o parcelamento em até " +
          internPlan.numeroMaximoParcelasPlano +
          " parcelas. Para ter um parcelamento maior, tente usar outro plano compatível com sua necessidade, ou solicite ao setor Administrativo/Financeiro para possível mudança de parcelamento deste plano."
      });
      ...
      try {
        internData.valorDesconto = (
          Number(internData.valorCurso) *
          (internData.descontoPlano / 100)
        ).toFixed(2);
        internData.valorAcrescimo = (
          Number(internData.valorCurso) *
          (internData.acrescimoPlano / 100)
        ).toFixed(2);
        internData.valorFinal = (
          Number(internData.valorCurso) +
          (internData.valorAcrescimo - internData.valorDesconto)
        ).toFixed(2);
        setRows([]);
        ...
        for (let parcela = 0; parcela < internData.numeroParcelas; parcela++) {
          ...
          if (internPlan.distribuirAcrescimosEDescontos === "on") {
            ...
            row =
              internPlan.quandoAplicar !== undefined
                ? {
                  ...
                  col4: ` ${descontoParcela !== 0 || descontoParcela !== ""
                      ? "- R$" + descontoParcela
                      : ""
                    }`,
                  col5: `R$${(
            ...
            row = {
              ...
              col2: `R$${parseFloat(internData.valorFinal / internData.numeroParcelas).toFixed(2)}`,
              col3: "0",
              col4: "0",
              col5: `R$${parseFloat(internData.valorFinal / internData.numeroParcelas).toFixed(2)}`
            };
    ...
    if (save) {
      ...
      if (
        (internData.vencimentoEscolhido && internData.numeroParcelas && internData["ano-mes"]) !==
        ""
      ) {
          ...
          <>
            ...
            <Container>
              <form
                ...
                <Grid justifyContent="flex-start" container direction="row" spacing={2}>
                  <Grid item>
                    <FormControl className={S.fields}>
                      <TextField
                        ...
                        id="nomeCursoAdd"
                        name="nomeCursoAdd"
                        value={data.nomeCursoAdd}
                        aria-describedby="my-helper-text"
                      />
                      <FormHelperText>
                        Identificação que aparece nos boletins e nos demais documentos emitidos pelo
                        sistema.{" "}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  <Grid item>
                    <FormControl className={S.fields}>
                      ...
                      />
                      <FormHelperText>
                        Código utilizado para formar os códigos automáticos de turma.
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                </Grid>
                ...
                <h6>
                  Todos os valores brutos estão em R$ (BRL - Brazilian Real / Real Brasileiro)
                </h6>
                <Grid justifyContent="flex-start" container direction="row" spacing={2}>
                  <Grid item>
                    <FormControl className={S.fields}>
                      ...
                      />
                      <FormHelperText>
                        Este é o nome de identificação do plano para a secretaria.{" "}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  <Grid item>
                ...
                <Grid justifyContent="flex-start" container direction="row" spacing={2}>
                  <Grid item>
                    <FormControl className={S.fields}>
                      ...
                      />
                      <FormHelperText>
                        O número máximo de parcelas é de{" "}
                        {plan.id && data.planos[plan.id].numeroMaximoParcelasPlano} parcelas.{" "}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  <Grid item>
                    <FormControl className={S.fields}>
                      ...
                      />
                      <FormHelperText>
                        Esta tabela serve apenas para visualizar a simulação de parcelamento no
                        contrato. Esta é a distribuição de parcelas para este plano.{" "}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                </Grid>
                  ...
                  <Grid item>
                    <FormControl className={S.fields}>
                      <TextField
                        ...
                        value={day}
                        type="text"
                        id="vencimentoEscolhido"
                        name="vencimentoEscolhido"
                        aria-describedby="my-helper-text"
                      />
                  ...
                  <Grid item>
                    <FormControl className={S.fields}>
                      <TextField
                        ...
                        autoComplete="off"
                        type="month"
                        label="Escolha quando começará o vencimento"
                      />
                      <FormHelperText>
                        {" "}
                        Escolha o mês e o ano para iniciar a geração dos boletos.{" "}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
...
function CourseDataFields(props) {
  ...
  const handleRowClick = (data) => {
    ...
    sessionStorage.setItem(
      activeStep,
      JSON.stringify({
        dadosTurma: coursesData.filter((course) => course.turmaAluno === data.id)[0]
      })
    );
    setOpenDialog(true);
  };
      ...
      <Grid justifyContent="center" container spacing={1}>
        ...
        <Grid item xs sm={3}>
          <Paper style={{ padding: "10px", alignItems: "center" }} elevation={5}>
            <h4>
              {courseChosen && courseChosen.turmaAluno !== "" ?
                <label>Turma escolhida: {courseChosen.turmaAluno}</label>
               :
            ...
            {contractState &&
              <Box>
                <FormControl className={S.fields}>
                  ...
                  <FormHelperText>
                    {" "}
                    Código gerado pelo sistema para o contrato. Não é necessário guardá-lo ou
                    alterá-lo.{" "}
                  </FormHelperText>
                </FormControl>
              </Box>
...
const AddressAndParentsFields = ({ shrink, parentsRequired, editMode = false }) => {
  ...
  const handleCloseDialog = () => {
    if (parentsFields.length === 0 && parentsRequired) {
      enqueueSnackbar("O aluno é menor de idade. Deve ser cadastrado pelo menos um responsável.", {
        variant: "warning",
        key: "0",
        action:
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <TextField
                      ...
                      name="nome"
                      aria-describedby="my-helper-text"
                      value={dialogOpen.nome}
                      onChange={(e) => {
                        setDialogOpen({ nome: e.target.value });
                      }}
                      FormHelperTextProps={{ error: true }}
                    />
                  </FormControl>
                ...
                <Grid item>
                  <FormControl variant="filled">
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        setDialogOpen({ relacao: e.target.value });
                      }}>
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <TextField
                      ...
                      name="celular"
                      aria-describedby="my-helper-text"
                      value={dialogOpen.celular}
                      onChange={(e) => {
                        setDialogOpen({ celular: e.target.value });
                      }}
                      FormHelperTextProps={{ error: true }}
                    />
                  </FormControl>
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <TextField
                      ...
                      onChange={(e) => {
                        setDialogOpen({ email: e.target.value });
                      }}
                      FormHelperTextProps={{ error: true }}
                    />
                  </FormControl>
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <TextField
                      ...
                      error={validCpf}
                      onBlur={(e) => {
                        e.target.value = validCpf ? "" : e.target.value;
                      }}
                      onChange={(e) => {
                        handleCheckCpf(e);
                        setDialogOpen({ cpf: e.target.value });
                      }}
                      type="text"
                      id="cpf"
                      name="cpf"
                      aria-describedby="my-helper-text"
                      value={dialogOpen.cpf}
                      helperText={validCpf && "Insira um CPF válido."}
                      FormHelperTextProps={{ error: true }}
                    />
                  </FormControl>
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <TextField
                      ...
                      value={dialogOpen.rg}
                      onChange={(e) => {
                        setDialogOpen({ rg: e.target.value });
                      }}
                      FormHelperTextProps={{ error: true }}
                    />
                  </FormControl>
                  ...
                  <FormControl className={S.fields}>
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          id="pedagogico"
                          checked={dialogOpen.pedagogico}
                          onChange={(e) => {
                            setDialogOpen({
                              pedagogico: e.target.checked,
                              financeiro: dialogOpen.financeiro
                            });
                          }}
                          name="pedagogico"
                          color="primary"
                  ...
                  <FormControl className={S.fields}>
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          id="financeiro"
                          checked={dialogOpen.financeiro}
                          onChange={(e) => {
                            setDialogOpen({
                              financeiro: e.target.checked,
                              pedagogico: dialogOpen.pedagogico
                            });
                          }}
                          name="financeiro"
                          color="primary"
                ...
                <Grid item>
                  <FormControl className={S.fields}>
                    <Button
                      variant="contained"
                      onClick={() => {
                        handleAddParentField(true, field.id);
                      }}>
                      Editar
                    </Button>
                  </FormControl>
