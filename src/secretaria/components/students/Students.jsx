...
const Students = () => {
  ...
  const handleRowClick = (e) => {
    ...
    setStudentInfo({
      id: e.id,
      classCode: e.row.turmaAluno,
      disabled: e.row.hasOwnProperty("disabled") ? true : false
    });
  };
  ...
  const handleShowDisabledStudents = (e) => {
    ...
    checked
      ? localStorage.setItem("showDisabledStudents", "yes")
      : localStorage.removeItem("showDisabledStudents");
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            {selectedRows.length > 1 ? "Serão" : "Será"} desativado{selectedRows.length > 1 && "s"}{" "}
            {selectedRows.length} aluno{selectedRows.length > 1 && "s"}.
          </DialogContentText>
        </DialogContent>
      ...
      <Grid justifyContent="flex-start" container direction="row" spacing={2}>
        ...
        <Grid item>
          <FormControlLabel
            control={
              <Checkbox checked={showDisabledStudents} onChange={handleShowDisabledStudents} />
            }
