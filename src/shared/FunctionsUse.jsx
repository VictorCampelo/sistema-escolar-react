...
const enrollStudent = async (studentData, classData, contractData, otherData) => {
  ...
  if (studentData.tipoMatricula === "preMatricula") {
    ...
    data = {
      dados: { ...studentData, ...classData, ...otherData },
      contratoConfigurado: contractData.contratoConfigurado,
      planoOriginal: contractData.planoOriginal,
      codContrato: contractData.codContrato
    };
  }
...
const handleTransferStudents = async (currentClass, destinationClass, studentsId) => {
  let data = {
    turmaAtual: currentClass,
    turmaParaTransferir: destinationClass,
    alunos: studentsId
  };
  let studentsArray = [];
  if (typeof studentsId === "string") {
...
async function createBilletView(
  parcelaAtual,
  numDeParcelas,
  vencimento,
  numeroDoc,
