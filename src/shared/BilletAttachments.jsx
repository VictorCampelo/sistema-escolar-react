...
const BilletAttachments = ({ docKeyPath, open, setOpen }) => {
      ...
      (error) => {
        ...
        switch (error.code) {
          case "storage/unauthorized":
            ...
            enqueueSnackbar("Você não possui permissões suficientes para enviar anexos.", {
              title: "Erro",
              variant: "error",
              key: "0",
              action: (
          ...
          case "storage/canceled":
            ...
            enqueueSnackbar("Upload cancelado.", {
              title: "Info",
              variant: "info",
              key: "0",
              action: (
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          ...
          billetsDocsRef
            .child(docKeyPath)
            .child("attachments")
            .push({ name: file.name, link: downloadURL, size: file.size, type: file.type });
          setProgress();
        });
      }
    );
  ...
  const handleDelete = async (name, databaseKey) => {
    try {
      ...
      enqueueSnackbar("Anexo deletado.", {
        title: "Sucesso",
        variant: "success",
        key: "0",
        action: (
                ...
                <div className={classes.demo}>
                  <List>
                    {attachments ? (
                        ...
                        <ListItem button onClick={() => handleOpenAttachment(i)}>
                          ...
                          <ListItemSecondaryAction>
                            <IconButton
                              edge="end"
                              aria-label="delete"
                              onClick={() => handleDelete(attachment.name, attachment.databaseKey)}
                            >
                              <DeleteForever />
                            </IconButton>
                          </ListItemSecondaryAction>
                      ...
                      <ListItem>
                        <ListItemText primary={"Não existem anexos para este boleto"} />
                      </ListItem>
                    )}
                  </List>
                  <Button
                    ...
                    onClick={() => document.getElementById("filesUpload").click()}
                  >
                    Novo anexo
                  </Button>
                  <input
